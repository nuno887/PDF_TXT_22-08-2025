...
PAIR_Y_TOL = 8.0       # sumário row pairing: max vertical distance (points) to link L<->R
...
# -------- NEW: Sumário with row pairing (L<->R) --------

def extract_sumario_block(lines, page_width):
    """
    Sumário extractor that tries to pair LEFT titles with RIGHT doc types
    by vertical proximity. Falls back to single-column if pairing not viable.
    """
    ok, split_x = two_col_ok(lines, page_width)
    if not ok or split_x is None:
        # Not a clean 2-col block → simple single-column read
        return extract_single(lines)

    left  = [(y0, (y0+y1)/2.0, x0, y1, t) for (y0, x0, y1, t) in lines if x0 < split_x]
    right = [(y0, (y0+y1)/2.0, x0, y1, t) for (y0, x0, y1, t) in lines if x0 >= split_x]
    left.sort(key=lambda it: (it[0], it[2]))   # by y0 then x0
    right.sort(key=lambda it: (it[0], it[2]))

    used_r = [False]*len(right)
    out = []

    # Greedy pairing: for each left line, attach nearest right by y-center within tolerance
    for y0L, yCL, x0L, y1L, tL in left:
        out.append(tL)
        # find nearest right by |y_center difference|
        j_best, d_best = -1, None
        for j, (y0R, yCR, x0R, y1R, tR) in enumerate(right):
            if used_r[j]:
                continue
            dy = abs(yCR - yCL)
            if dy <= PAIR_Y_TOL and (d_best is None or dy < d_best):
                d_best, j_best = dy, j
        if j_best >= 0:
            out.append(right[j_best][4])
            used_r[j_best] = True

    # Append any unpaired right items (rare, but can happen due to wraps)
    for j, used in enumerate(used_r):
        if not used:
            out.append(right[j][4])
        ...
        for i in range(doc.page_count):
            ...
            if in_sumario:
                if ycut is not None:
                    ...
                    # Sumário with pairing
                    sumario_parts.append(extract_sumario_block(top, page_w))
                    # bottom may be 2-col or single
                    ok, split_x = two_col_ok(bot, page_w)
                    body_parts.append(extract_two_col(bot, split_x) if ok and split_x else extract_single(bot))
                    in_sumario = False  # Sumário ended at this split
                else:
                    # Entire page still Sumário or ambiguous → keep as Sumário with pairing attempt
                    sumario_parts.append(extract_sumario_block(lines, page_w))
            else:
                # After Sumário → normal body pages
...
def main():
    ap = argparse.ArgumentParser(
        description="Extract Sumário first (paired L<->R rows if 2-col), then body; handles mid-page switch."
    )
